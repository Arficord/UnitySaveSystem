...
namespace Arficord.SavingSystem
{
    public class SaveSystem : MonoBehaviour
    {
        ...
        private void Awake()
        {
            for (int i = 0; i < savers.Length; i++)
            {
                ...
                {
                    Debug.LogError(
                        $"Saver key [{saver.Key}] already present in savers dictionary. Check if save system contains savers with identical keys.");
                    continue;
                }
        ...
        public void Delete(string storeKey)
        {
            ...
            if (store == null)
            {
                Debug.LogError($"Can't find save store with [{storeKey}] key");
                return;
            }
        ...
        public void Delete(SaveStore store)
        {
            store.DeleteSave();
        }
        ...
        private void ApplySaveData(SaveSystemSaveData saveData, SaveSerializer saveSerializer)
        {
        ...
        private SaveSystemSaveData CollectSaveData(SaveStore store, out SaveSerializer saveSerializer)
        {
            ...
            {
                Debug.LogError(
                    $"Store save data contains save serializer key [{saveDataString.SerializerKey}] which is not presented in Save System");
                return null;
            }

            if (saveSerializer.TryDeserialize(out SaveSystemSaveData saveData, saveDataString.SaveData))
            {
                return saveData;
            }
